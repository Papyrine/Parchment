...
3. `SettingsProtection.Apply` writes `w:documentProtection w:edit="readOnly" w:enforcement="1"` into the settings part at **registration** (canonical bytes), gated on `!map.IsEmpty && protection == ProtectionMode.WhenEditable`. Passwordless by design (determinism + no real security either way). Immediately after, gated additionally on `map.HasHtmlField` (any `Html`-kind entry, recursing into collection element maps), `WordNumbering.EnsureListDefinitions` seeds a bullet + decimal numbering definition. Word can only apply a list against a pre-existing definition — creating one writes the document-level `numbering.xml`, which read-only protection locks — so without the seed the list buttons are greyed out inside an otherwise-editable rich-text block. Seed uses the deterministic `rNumbering` rel id (byte-reproducible); idempotent, so a later render whose HTML contains a real list allocates ids past it.
