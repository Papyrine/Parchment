...
`netstandard2.0;net10.0`, packaged inside `Parchment.nupkg` under `analyzers/dotnet/roslyn5.0/cs` and `analyzers/dotnet/roslyn5.3/cs`. `IIncrementalGenerator` + `ForAttributeWithMetadataName("Parchment.ParchmentModelAttribute")`. The attribute is parameterless — the template is found by **convention**: the AdditionalFile named after the type (`Invoice.docx` or `Invoice.md`; both/none → PARCH020/PARCH004, matching in `TemplateConvention.MatchByTypeName`). Same attribute supports **both** docx and markdown — SG branches on the matched file's extension. Pipeline collects `targets`, `docs`, `markdowns`, `dotxes` in four parallel `AdditionalTextsProvider` stages (each cached separately for incrementality), `Combine`s them, dispatches per target.
